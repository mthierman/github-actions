...
function getInstallRoot(version: string, asset_name: string): string {
    const digest = createHash("sha256").update(`${version}|${asset_name}`).digest("hex").slice(0, 12);
...
(async () => {
    ...
    try {
        ...
        if (!restored_key) {
            ...
            const install_result = spawnSync(installer_path, ["/VERYSILENT", "/CURRENTUSER", `/DIR=${install_dir}`], {
                stdio: "inherit",
            });
            ...
            if (install_result.status !== 0) {
                throw new Error(`Inno Setup installation failed with code ${install_result.status}`);
            }
